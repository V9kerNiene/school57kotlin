...
```bash
grep -r "ERROR" ./logs/*.logs ./logs/old/*.logs > errors.txt
```
...
```bash
find ./logs -name "*.logs" -printf '%s\t%p\n' | sort -rn | head -n 1 | cut -f2- > biglog.txt
```
...
```bash
find logs -name "*.logs" | wc -l > log_count.txt
...
```bash
zip -r config_backup.zip config/*.conf
```
